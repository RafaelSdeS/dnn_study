...
scripts/                  # CLI entry points (used instead of notebooks for PCAD/cluster runs)
  ...
  backfill_best_epoch_eval.py  # One-off: re-evaluates FP32 (+ rebuilds INT8 where a full-precision QAT-best
                           #   checkpoint survived) from saved best checkpoints, for models trained before
                           #   the Trainer.fit() checkpoint-restore fix (see Trainer note above)
  ...
  pcad/                    # PCAD submission wrappers
    ...
    submit_phase7_simple.sh / submit_phase7_multinode.sh  # PCAD Phase 7 detection submission (simple vs FP32→QAT→INT8 chaining) — see research/logs/PHASE7_MULTINODE.md
...
results/                  # tracked CSVs/JSON/figures, one dir per phase + results_aggregate/
                          #   figures_generated/phase_*/ subdirs are hand-sorted from the flat notebook
                          #   output (see FIGURES_DIR in each analysis notebook) — that move isn't scripted,
                          #   so a fresh notebook rerun repopulates the flat files but not the phase dirs
...
research/                 # docs/ + ideas/ merged, split by kind
...
jupyter lab
```
`requirements.txt` (pip freeze, tightly pinned) is the source of truth for `.venv` — it has drifted before (`research/logs/PHASE7_LOG.md`); re-run the `pip install -r requirements.txt` line if notebook imports start failing. `environment.yml` is separate and only for the conda env below.
