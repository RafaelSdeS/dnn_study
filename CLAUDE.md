...
| 11 — Kernel size comparison | `models/baselines.py` | `AlexNetTV(kernel_size=None\|3\|2)` (original 11×11/5×5/3×3, 3×3, 2×2) and `VGG16(kernel_size=3\|2)` (torchvision cfgs["D"], no BN, kernel_size=3 is VGG's own native design) — all 5 trained from scratch, no early stopping, via `configs/experiments/phase_11_kernel_size_comparison.yaml` (`_protocols/no_patience.yaml`: 500ep FP32 / 100ep QAT) |
