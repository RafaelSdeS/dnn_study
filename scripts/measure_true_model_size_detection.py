...
Detection counterpart to scripts/measure_true_model_size_segmentation.py -- same bug, same fix.
The existing `summary.model_size_mb` (from compute_detection_summary / scripts/backfill_int8_size.py)
...
Usage:
    python -m scripts.measure_true_model_size_detection
"""
...
def measure(phase7_dir: Path) -> None:
    ...
    for run_dir in sorted(phase7_dir.glob("ssd_*")):
        ...
        if m.group("stage") == "fp32":
            ...
            # script does at eval time -- see scripts/backfill_int8_size.py.
