...
- `winograd_conv2d_f23(x, weight)` / `profile_layer_conv_winograd(kernel_size, in_ch, out_ch, input_size, device, warmup=50, iters=200)` — a **real** Winograd F(2×2,3×3) transform (fixed Lavin & Gray `G`/`B^T`/`A^T` matrices, not a simulation): tiles the padded input into overlapping 4×4 blocks, transforms input and kernel into the Winograd domain, elementwise-multiplies (16 multiplies/tile vs. GEMM's 36), inverse-transforms to 2×2 output tiles. Correctness is verified in `tests/test_profiling.py` against `F.conv2d` directly (a sign/transpose error in the transform matrices gives a plausible-but-wrong number, not a crash, so this is load-bearing). Only valid for `kernel_size=3`, stride=1, groups=1 — mirrors `profile_layer_conv_fft`'s `kernel_size >= 5` restriction on the other side. This gives H1/H2 a third, most direct signal: unlike `winograd_trace_detected` (cuDNN kernel-name heuristic) and `winograd_speedup_info` (empirical 3×3-vs-5×5 ratio, an indirect proxy for *whatever* cuDNN picked), this measures the actual reduced-multiply-count algorithm's wall-clock cost — the *algorithmic* FLOP-reduction claim, separate from whether cuDNN's specific fused kernel realizes it in hardware. A naive eager-mode implementation (3+ separate kernel launches: input transform, elementwise multiply, output transform) can lose to cuDNN's single fused kernel even while doing fewer FLOPs — that gap is itself part of what's being measured, not a bug to fix. Output kind `winograd_custom` in the layer sweep (kernel_size=3 only, one config per `layer_channels` width, fixed batch=1/res=64 like the FFT sweep), sitting next to `kind: layer` (cuDNN's own algorithm choice) for a direct per-width comparison.
